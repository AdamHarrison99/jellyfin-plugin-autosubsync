...
**Filtered before fetching, so it costs nothing:** an advertised-forced candidate (signs and songs cannot answer for the language), an advertised AI- or machine-translated one, **a name that says the file is hearing-impaired**, a format the engine cannot read, and any id already in this row's ledger. ! `null` on those flags means *the provider did not say*, ¬*no*.

#### `SdhNaming`, and why the name is worth reading

The advertised `HearingImpaired` flag is **not reliable**. Measured over 703 real downloads from one provider: **every one carried `IsSdh: false`**, and the byte-level `SdhDetector` then called **454** of them hearing-impaired. Of the 340 whose filename survives in the ledger, **50 say `SDH` in the name** — the provider served a file called `….eng.SDH.srt` while reporting the flag as false. ! The 703 all reading false is **¬ evidence of mislabelling on its own** — an advertised-SDH offer is dropped before any fetch, so every *downloaded* candidate has the flag false by construction. The filename is the independent signal, and the control is that of the candidates which got **past** the SDH check to the audio check, **¬one** had `SDH` in its name.

→ reading the name is worth a free filter, ∵ it converts a paid discard into an unpaid one for the ~15% that say so. Real shapes, all observed in one day: `….eng.SDH.srt` (dominant) · `English SDH <release>` · `en[sdh].srt` · `.DVD.HI.pcc.` · `eng-HI`.

! **`SDH` and `HI` cannot take the same rule.** `SDH` spells no English word → any non-alphanumeric delimiter is safe. `HI` does → it is matched **upper-case only, w/ punctuation on one side**, ∵ `RemoteSubtitleInfo.Name` is not always a filename: the Addic7ed plugin builds it as `[<version>] <show> | <episode title> | Completed`, so a lower-case `Hi` between spaces is an episode title, ¬a marker. Both real hits (`.HI.`, `-HI`) satisfy the stricter rule and no name observed collides.

! **A token can mean the opposite.** `English (SDH Removed) <release>` was offered the same day — the annotations are **gone**, which makes it the *best* file on the list. A `contains("SDH")` refuses exactly the candidate it should prefer. `sdh|hi` qualified by *removed*, *stripped*, *no*, *non* or *without* is not a marker.

! **What `Name` actually holds, verified against each whitelisted plugin's source** — subbuzz sets it from `file.Name`/`file.FileName`/`FileTitle` per internal source (a real filename, carries the token); Jellyfin's OpenSubtitles plugin sets it to `Attributes.Release` (a release name, and that plugin populates `HearingImpaired` properly, so the free flag filter already covers it); the Addic7ed plugin builds the label above and will never match. → the check helps subbuzz, is harmless elsewhere, and must never assume a filename.
...
! **`RemoveHearingImpairedTags` does ¬widen what the acquire path accepts.** Stripping is a *transform* applied to a file the library already has or has already been allowed to keep; it is ¬a licence to buy one. `AcquireHearingImpaired` alone decides whether an SDH candidate is acceptable, both for the free pre-filter and for the post-fetch discard → a user who wants SDH downloads turns that setting on and nothing else implies it.

! **A budget spent entirely on SDH is a skip, ¬a failure.** Post-fetch discards trip `MaxDownloadsPerItem` **before** the loop can fall through to `HearingImpairedOnly` → the item reported *the download limit was reached before a subtitle could be confirmed against the audio* when the audio check had seen nothing at all. The cap branch tests the tally first: no refusal and no failure, w/ discards on the board, is `HearingImpairedOnly` and carries its own message. A cap w/ even one genuine refusal in it stays `CapReached`.

! **A skip is ¬ gated by the settings stamp, so the budget has to gate itself.** `IsExhausted` requires `Status == Failed` → the moment the all-SDH cap became a skip, the row stopped being exhausted and bought `MaxDownloadsPerItem` fresh candidates on **every** scan, for ever, ∵ the ledger only blocks the ids already tried. `SyncOrchestrator.BudgetSpent` closes it: a `SetAside` row whose `AcquireAttempts` count has reached the limit is returned before the search, ¬only before the fetch — a search is a network request under the admin's account. ! The release is **raising the limit**, ¬the *Retry failed subtitles* button, which reopens `Failed` alone; the setting that stopped the row is the setting that restarts it, and the ledger keeps the second budget off ids already bought.

#### The abstention, and who decides what it means

An `Inconclusive` verdict is the check reaching **no answer**, ¬a verdict against the file. `RequireConclusiveDownloads` (**on** by default) decides what that means for a **downloaded** candidate. On, the abstention is charged to the candidate like any other refusal, the loop falls through, and the item keeps buying until the check is conclusive or `MaxDownloadsPerItem` runs out. ! **The budget is the only thing bounding patience** — nothing else stops an item whose audio can never be measured from spending its whole limit every time it is offered.

**Off, the first candidate nothing refused is kept.** The setting is ¬a branch in the acquire loop — a copy there would stop the item *ahead* of the gates. It is passed into `SyncDecisionMaker.Decide` as `requireConfirmation`, so an abstention falls through to the gates **below** the verdict instead of ending on it: the `MaximumUnverifiedShiftMs` ceiling first, then `MinimumEngineScore`. ! **"Nothing refused it" is every gate, ¬the verdict alone.** A download the engine never scored, or scored under the floor, is still refused and the loop still falls through — what the setting keeps is a candidate nothing rejected, ¬merely one the audio check could not measure.

! **It rides on `RequireAudioConfirmation` and can never loosen it.** `SyncOrchestrator.DownloadNeedsConfirmation` is the two `&&`ed → w/ the sync gate off the download setting does nothing in either direction, ∵ an abstention is already eligible for acceptance on the engine's own score. The override is passed by the **acquire** caller alone; the sync path leaves it null and goes on reading the sync setting.

! **Turning it off is a deliberate relaxation of the third download invariant** (→ `CLAUDE.md`). *Nothing unconfirmed by the video's own audio reaches the library* holds under the default; the setting is what trades it for coverage on a library the check cannot measure, and the engine-score floor is what stays standing underneath.

! **A budget spent entirely on abstentions is named for the abstention, ¬the refusal.** The `Exhausted` message is chosen on the tally: `Failures == 0 && Abstentions == Refusals` → `NoVerdictExhausted`, otherwise the every-candidate-was-refused wording. Both are refusals and both are `Failed`; only the card differs. ∵ the loop charges an abstention to the candidate exactly like a misalignment, the tally is the **only** place the two are still distinguishable by the time the message is written.

! **The ledger records the verdict, ¬what the setting did with it.** `Map(verdict)` still writes `Inconclusive`. Folding it into `Misaligned` at the judge would have made the field evidence unreadable — *the check refused this file* and *the check could not read this title* are the two things the ledger exists to tell apart.
...
! **Retire the source, ¬the aggregator.** subbuzz is several accounts behind one provider name, so one spent allowance inside it used to end the whole provider's list for that item. `RetireSource` keys a second table on the pair and `ReasonFor(provider, source)` reads the whole-provider key **first**, ∵ a walled provider carries every source under it. The loop's per-offer check costs nothing and runs before the fetch → a walled source's remaining offers are skipped rather than bought, and only a wall the **provider** is behind still `break`s the list.

! **A search-time wall still retires the whole provider.** Nothing is in flight during a search, so there is no source to charge it to; inventing one would wall a source that never answered. The narrowing applies to `FetchAsync` alone, where a candidate names its source.

**Which source produced a result** → `SubtitleSourceKey`. An aggregator stamps the source in the **result id** (`<plugin-guid>_<source><payload>`) and again in the labels beside it — a bracketed token, a bolded name, sometimes both. ! **A label alone is never a key.** Candidate tokens come from `Comment` and `ProviderName`, markup stripped and both halves of a `[…] rest` split offered, and each is used only where the **id** confirms it: the tail after the first underscore must start with it. Nothing confirmed → null → whole-provider behaviour. ∵ the label is free text and the id is what the fetch that hit the wall was actually made through, an unconfirmed token would wall the wrong account.

! **`Name` is excluded and the token must be host-shaped** — dotted, no whitespace. Both guard the **costly** direction. An aggregator sets `Name` from the candidate's filename (→ *what `Name` actually holds*), and any label that happens to prefix its own id invents a source; a wall then narrows to that invention instead of stopping the provider, and a spent account goes on being asked for every offer behind it. Every source these aggregators name is a host, so the shape test costs nothing real, and a dotless source degrades to whole-provider retirement — the failure that lands on the safe side.

! **A list left entirely behind a walled source is `ProvidersRetired`, ¬`AllFiltered`.** Those candidates were usable; the allowance is what stopped them, and *no subtitle offered for this language could be used* blames the files. The branch is **last**, so anything the check actually decided about a fetched file outranks it.
...
**The *downloaded* card counts survivors; the `Acquire` stage row counts items, exactly as every other row does.** The row appears only while `AcquireMissingSubtitles` is on, exactly as `Convert` does for OCR. See *The status panel invariant* in `CLAUDE.md`: any split beyond that one is how `FAILED` came to disagree w/ *failed*.

! **It read the ledger once, and the ledger counts downloads.** `Succeeded` and `Failed` came off `AcquireAttempts` while `Skipped` came off the stages → **Done and Failed were fetches, Skipped was items**, on a table whose every other row is items, and the three summed to nothing. Worse, a fetch is *not Kept* the moment a setting discards it, so the 671 hearing-impaired files the plugin correctly set aside were reported as **1108 failed downloads** — the same class of error as the cap outcome, on the row that shows it to the user. ! **A refusal is ¬a failure, and `Acquire` can hold one**: the outcome carries `RefusedByAudio` onto the row, so the `IsAudioRefusal` exclusion covers `Verify` **and** `Acquire`, ¬`Verify` alone. The line is drawn once. **What the allowance actually bought is no longer on the panel at all** — it is in the ledger, and nothing renders it.
...
`SummarizeStages` walks a **fixed pipeline description** rather than the stages present in the store, emitting a row only where the corresponding setting is on: `Convert` absent unless `ConvertImageSubtitles`, `Transform` unless `RemoveHearingImpairedTags`, `Deduplicate` unless `DeduplicateSubtitles`; `Sync` unconditional. `Acquire` leads the description, present only while `AcquireMissingSubtitles` is on — acquisition runs before anything else the pipeline does to a subtitle. ! **It counts items like every other row.** It shipped w/ a hand-rolled row that counted *fetches* on the Done and Failed cells and *items* on Skipped, ∵ the ledger was the obvious thing to count; a library whose downloads were mostly SDH discards then read **1108 failed** against four failed items. A row that counts a different noun than the row above it is the panel lying by units.

**The refusal split lives on the cards and the reason lists, ¬on the stage table.** `SyncOutcome.IsAudioRefusal` decides both: it separates the *failed* card from *rejected by audio check*, and it separates `FailureReasons` from `RefusalReasons`, which render as two blocks. ! **The refusals then split once more, and only there.** `IsInconclusiveRefusal` lifts the abstentions onto their own *rejected as inconclusive* card and reason block, ∵ a **setting** raised those and nothing else in the bucket — folded in w/ the rest they read as the check's own doing, and the user cannot tell which tile *Only sync when the audio checks are conclusive* would empty. It is a partition of the refusals, ¬a third source of truth: `IsAudioRefusal` still decides refused-versus-failed, once, and the inconclusive card is subtracted from the one beside it so the two never double-count. ! A single list over both statuses is what shipped first, and its total matched neither card — a `failed 1` tile above a list summing to 191. Both lists go through one `Reasons(...)` helper so a change to the grouping cannot reach one and miss the other.
...
**All three reason lists are bounded at `ReasonLimit = 100`**, high enough never to cut a real list — the field store's 331 refusals produce four rows — and kept only as a valve for the J3 case, where a message stops collapsing into its group and renders one row per subtitle. ! The earlier bound was **8**, low enough to truncate a live list silently so it totalled less than the card above it, which is the D1/E3 shape twice fixed → K9. `UnsupportedReasons` was unbounded and now shares the limit.
