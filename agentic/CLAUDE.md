...
- **Dry run is a media-filesystem lock**, ¬a logging mode. When on, no write to the media library happens; the plugin's own record store is still written. On by default on a fresh install.
