...
> Finding IDs are `<letter><n>`, one letter per pass, and a letter is never reused — `H1` means one thing forever. **Used: A B C D E F G H J K L M N P Q R S T U V W X Y Z AA AB AC. Take the next pass's from `AD`.** ! `I` and `O` are held back ∵ `I1`/`O1` read as `11`/`01`; when the pool runs out, go to `AA`, ¬to those.
